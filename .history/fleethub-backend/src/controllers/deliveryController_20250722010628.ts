...
export async function assignDelivery(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    if (!driverId || !truckId) {
      res.status(400).json({ error: 'driverId and truckId are required' });
      return;
    }
    // Prevent assigning a driver to multiple in_progress deliveries
    const activeDelivery = await deliveryService.getActiveDeliveryForDriver(driverId, deliveryId);
    if (activeDelivery) {
      res.status(400).json({ error: 'Driver is already assigned to another active delivery.' });
      return;
    }
    // Fetch delivery to get distance
    const delivery = await deliveryService.getDeliveryById(userId, deliveryId);
    if (!delivery) {
      res.status(404).json({ error: 'Delivery not found' });
      return;
    }
    // Calculate ETA: 1 min per 100km (or use your own logic)
    const now = new Date();
    const etaMinutes = Math.ceil(delivery.distanceKm / 100);
    const expectedCompletionTime = new Date(now.getTime() + etaMinutes * 60 * 1000);
    const updatedDelivery = await deliveryService.updateDelivery(
      userId,
      deliveryId,
      {
...
export async function getSupportedCities(req: AuthRequest, res: Response): Promise<void> {
  try {
    console.log('getSupportedCities called');
    const cities = getAllCities();
    console.log('Returning cities:', cities);
    res.json(cities);
  } catch (err: any) {
    console.error('Error in getSupportedCities:', err);
    res.status(500).json({ error: err.message });
  }
}