    ...
    <MainLayout>
      <div className="trucks-list">
        ...
        <div className="trucks-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '2rem', marginTop: '2rem' }}>
          {trucks.map(truck => (
            <div key={truck.id} className="truck-card" style={{ background: '#fff', borderRadius: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.08)', padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
              <img
                src={truck.photoUrl}
                alt="Truck"
                style={{ width: 180, height: 120, borderRadius: 12, objectFit: 'cover', marginBottom: 16, boxShadow: '0 1px 6px rgba(0,0,0,0.10)' }}
              />
              {/* Remove truck name and status */}
              {/* <h2 style={{ margin: '0 0 8px 0', fontSize: 22 }}>{truck.model}</h2> */}
              {/* <div style={{ marginBottom: 8 }}>
                <span className={`status-badge status-${truck.status?.toLowerCase()}`}>{truck.status?.replace('_', ' ')}</span>
              </div> */}
