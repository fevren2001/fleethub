...
const TRUCK_PHOTOS = [
  '/src/assets/truck_photos/truck1.webp',
  '/src/assets/truck_photos/truck2.jpg',
  '/src/assets/truck_photos/truck3.webp',
  '/src/assets/truck_photos/truck4.webp',
];
...
const TruckForm: React.FC<TruckFormProps> = ({ initialData = {}, onSubmit, isSubmitting }) => {
  const [formData, setFormData] = useState({
    ...
    photoUrl: initialData.photoUrl || TRUCK_PHOTOS[0],
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.garageId) {
      setFormError('Please select a garage.');
      return;
    }
    if (initialData.id) {
      // Edit mode
        ...
        >
          ...
          {garages.map((garage) => (
            <option
              key={garage.id}
              value={garage.id}
              disabled={garage.occupancy >= garage.capacity}
            >
              {garage.id} - {garage.city} {garage.occupancy >= garage.capacity ? '(Full)' : ''}
            </option>
          ))}
        </select>
      </div>
