...
const TruckDetail: React.FC = () => {
  ...
  const handleStatusChange = async (newStatus: Truck['status']) => {
    if (!id || !truck) return;
    
    setStatusUpdating(true);
    
    try {
      await updateTruck(parseInt(id, 10), { status: newStatus });
      ...
      <div className="truck-detail">
        ...
        <div className="detail-card">
          <div className="detail-row">
            ...
            <select
              value={truck.status}
              onChange={(e) => handleStatusChange(e.target.value as Truck['status'])}
          ...
          {truck.garageId && (
            <div className="detail-row">
              ...
              <span>Garage ID: {truck.garageId}</span>
            </div>
          )}
          ...
          {truck.driverId && (
            <div className="detail-row">
              ...
              <Link to={`/drivers/${truck.driverId}`}>View Driver</Link>
            </div>
          )}
