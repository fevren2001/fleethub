...
export const updateTruck = async (model: string, truckData: Partial<TruckFormData>): Promise<Truck> => {
  const { model, garageId } = truckData;

  const response = await apiClient.put<Truck>(`/trucks/${id}`, { name, skill });
  return response.data;
};
