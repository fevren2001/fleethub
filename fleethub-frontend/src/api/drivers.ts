...
export const updateDriver = async (id: number, driverData: Partial<DriverFormData>): Promise<Driver> => {
  const { name, skill, photoUrl } = driverData;
  const response = await apiClient.put<Driver>(`/drivers/${id}`, { name, skill, photoUrl });
  return response.data;
};
