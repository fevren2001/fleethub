...
export async function addDelivery(req: AuthRequest, res: Response): Promise<void> {
  try {
    ...
    res.status(201).json({
      ...newDelivery,
      estimatedDurationMinutes: deliveryService.calculateEstimatedCompletionTime(distanceResult.distanceKm)
    });
  } catch (err: any) {
...
/** GET /deliveries/:id/progress */
export async function getDeliveryProgress(req: AuthRequest, res: Response): Promise<void> {
  try {
    const userId = req.user!.userId;
    const deliveryId = parseInt(req.params.id);
   
    const progress = await deliveryService.getDeliveryProgress(userId, deliveryId);
    res.json(progress);
  } catch (err: any) {
    res.status(400).json({ error: err.message });
  }
